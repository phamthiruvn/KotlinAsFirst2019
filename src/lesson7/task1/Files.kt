...
fun top20Words(inputName: String) =
    File(inputName).readLines().map { it.toLowerCase() }.fold(listOf("")) { result, line ->
        result + Regex("""[A-Za-zа-яА-ЯЁё]+""").findAll(
            line,
            0
        ).map { it.value }
...
fun markdownToHtmlSimple(inputName: String, outputName: String) {
    val e = File(inputName).readLines().toMutableList().map{ if (it == "") "</p><p>" else it }.joinToString("")
    val outputStream = File(outputName).bufferedWriter()
    var a = 0
    var b = 1
    var s = 1
    ...
    outputStream.write(result.joinToString("").replace("</p><p></p><p>", "</p><p>"))
    outputStream.close()
}
