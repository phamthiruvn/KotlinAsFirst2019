...
fun subtractOf(a: MutableMap<String, String>, b: Map<String, String>) {
    for (i in b.keys) if (a[i] == b[i]) a.remove(i)
}
...
fun canBuildFrom(chars: List<Char>, word: String) =
    word == "" || chars.map { it.toUpperCase() }.containsAll(word.toUpperCase().toSet())
 ...
 */
fun extractRepeats(list: List<String>) = list.groupBy { it }.mapValues { it.value.size }.filterValues { it != 1 }
...
fun bagPacking(treasures: Map<String, Pair<Int, Int>>, capacity: Int): Set<String> {
    ...
    for (i in 0..treasures.size) {
        ...
        for (j in 0..capacity)
            array[i].add(Pair(mutableSetOf(), 0))
    }
    for (i in 0..treasures.size) {
        for (j in 0..capacity) {
            if (i == 0 || j == 0)
                array[i][j] = Pair(mutableSetOf(), 0)
            else if (bec[i - 1] <= j) {
                array[i][j] = Pair(
    ...
    return array[treasures.size][capacity].first
}